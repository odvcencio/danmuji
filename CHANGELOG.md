...
- Upgraded gotreesitter from v0.20.5 to v0.53.0 and regenerated `language.bin`/`language.hash`. The grammar's JSON export (`testdata/grammar.json`) is byte-identical; the compiled blob shrank from 881,643 to about 135,000 bytes as a result of gotreesitter's own generated-table changes, not a grammar change.
